...
const bulkItemValidate = (data) => {
  if (!("Category Name" in data)) {
    return `Error in cell Category ${data["Category Name"]} & Item : ${data.Item}, Column 'Category Name' : Blank Not Allowed.`;
  }

  if (data["Category Name"].length > 42) {
    return `Error in cell Category ${data["Category Name"]} & Item : ${data.Item}, Column 'Category Name' :  Maximum allowed characters are 42.`;
  }

  if (!("Item" in data)) {
    return `Error in cell Category ${data["Category Name"]}, Item : ${data.Item}, Column 'Item' : Blank Not Allowed.`;
  }

  if (data.Item && data.Item.length > 42) {
    return `Error in cell Category ${data["Category Name"]}, Item : ${data.Item}, Column 'Item' : Maximum allowed characters are 42.`;
  }

  if (data["Item Code"] && data["Item Code"].length > 42) {
    return `Error in cell Category ${data["Category Name"]} & Item : ${data.Item}, Column 'Item Code' : Maximum allowed characters are 42.`;
  }
  ...
  if (
    ...
    return `Error in cell Category ${data["Category Name"]} & Item : ${data.Item}, Column 'ItemMode' : Only Values allowed are 0 , 1 and 2.`;
  }
  if (
    ...
    return `Error in cell Category ${data["Category Name"]} & Item : ${data.Item}, Column 'GST%' : Values allowed upto 3 Digits only.`;
  }
  ...
  if (
    ...
    return `Error in cell Category ${data["Category Name"]} & Item : ${data.Item}, Column 'VAT%' : Values allowed upto 3 Digits only.`;
  }

  if (
    "Parcel Type" in data &&
    !(data["Parcel Type"] == "1" || data["Parcel Type"] == "2")
  ) {
    return `Error in cell Category ${data["Category Name"]} & Item : ${data.Item}, Column 'Parcel Type' : Only Values allowed are 1 or 2.`;
  }
  ...
  if (
    ...
    return `Error in cell Category ${data["Category Name"]} & Item : ${data.Item}, Column 'Exclude from Discount' : Only Values allowed are 0 or 1.`;
  }
  ...
  if (
    ...
    return `Error in cell Category ${data["Category Name"]} & Item : ${data.Item}, Column 'Exclude from Charge' : Only Values allowed are 0 or 1.`;
  }

  if ("Status" in data && !(data["Status"] == "0" || data["Status"] == "1")) {
    return `Error in cell Category ${data["Category Name"]} & Item : ${data.Item}, Column 'Status' : Only Values allowed are 0 or 1.`;
  }
        ...
        if (dataValidate == true) {
          ...
          if (!categoryResult) {
            if (!("Brand UUID" in data[i]) || data[i]["Brand UUID"] == "") {
              error.push(
                `Error in cell Category ${data[i]["Category Name"]} & Item : ${data[i].Item}, Column 'Brand UUID' : Brand UUID is mandatory for New item / Category.`
              );
            } else {
                ...
                if (menu) {
                  ...
                  for (let i = 0; i < menu.length; i++) {
                    ...
                    const categoryItem = JSON.parse(
                      JSON.stringify(menuResult.category_and_items)
                    );
                    for (let j = 0; j < categoryItem.length; j++) {
                      if (categoryItem[j].category_uuid == category_uuid) {
                        const menuItem = JSON.parse(
                          JSON.stringify(categoryItem[j].menu_items)
                        );
                        for (let k = 0; k < menuItem.length; k++) {
                          if (menuItem[k].item_uuid == merge.item_uuid) {
                            categoryItem[i].menu_items[k][
                              "menu_item_parcel_charge"
                            ] =
                              data[i]["Parcel Charge"] ||
                              menuItem[k]["menu_item_parcel_charge"];
                            categoryItem[i].menu_items[k]["menu_item_price"] =
                              data[i]["Price"] ||
                              menuItem[k]["menu_item_price"];
                            categoryItem[i].menu_items[k][
                              "menu_item_description"
                            ] =
                              data[i].description ||
                              menuItem[k]["menu_item_description"];
                          }
                        }
                      }
                    }
