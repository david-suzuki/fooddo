...
import { menuType, categoryType, itemType, menuItemType, categoryMenuType } from "../types";
...
export default function MenuPage(props: {seatUUidHandler: (seat_uuid:string) => void}) {
    ...
    const category_data = { outlet_uuid: 'outlet-uuid-1' };
    const menu_data = { outlet_uuid: 'outlet-uuid-1', menu_uuid: 'menu-uuid-1' };

    const navigate = useNavigate();
    
    useEffect(() => {
        axios.post(remoteServerUrl + "menu/getMenu", menu_data)
        .then((response) => {
                    ...
                    <div className="customizeMenu">
                        <div className="menuCus">
                            <div className="itemDetails" style={{ display: 'inline'}}>
                                <img src={items.find((i) => i.item_uuid === customItem?.item_uuid)?.item_mode === "1" ? "avilable.png" : "notAvilable.png"} alt="veg" />
                                <span style={{ marginLeft: '10px' }}>{ items.find((item) => item.item_uuid === customItem?.item_uuid)?.item_name }</span>
                            </div>
                            ...
                            customItem?.menu_item_addons.map((citem, i) => 
                                <div key={i} style={{ width: '100%' }}>
                                    ...
                                    { citem.addon_item_details.map((detail, k) => {
                                        return <div className="customOptions" key={k}>
                                            <div className="csoption">
                                                <h4>{items.find((item)=>item.item_uuid === detail.item_uuid)?.item_name}</h4>
                                                <div className="prAndSel">
                                                    <h4>{currency}{detail.price}</h4>
                                                    <input
                                                        id={detail.item_uuid} 
                                                        type="checkbox"
                                                        name={items.find((item)=>item.item_uuid === detail.item_uuid)?.item_name} 
                                                        value={detail.price} 
