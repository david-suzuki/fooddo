...
import { categoryMenuType, menuItemType } from "../types";
...
export default function Dinein(props: {seatUUidHandler: (seat_uuid:string) => void}) {
    ...
    useEffect(() => {
        if (qrCode !== '') {
            var seat_uuid = qrCode.split("/").at(-1)
            axios.post(remoteServerUrl + "seat/getMenu", {seat_uuid: seat_uuid})
            .then((response) => {
                var category_menus = response.data.category_menus
                setCategoryMenus(category_menus)
    ...
    const toCartHandler = (event: React.MouseEvent<HTMLElement>) => {
        var seat_uuid = qrCode.split("/").at(-1)
        var value = seat_uuid !== undefined ? seat_uuid : ""
        props.seatUUidHandler(value)
        navigate('/cart');
    }
